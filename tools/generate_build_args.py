...
that produce a Chiseled application image. Downstream build pipelines should
...
def build_invocation(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    build_args["UBUNTU_BUILDER_IMAGE"] = manifest["builder"]["image"]
    build_args["CHISEL_VERSION"] = chisel["cli"]["version"]

    for arch in ARCH_ORDER:
        if arch in image_arches:
            digest = chisel["cli"]["sha384"].get(arch)
            if digest is None:
                raise GenerateError(
                    f"chisel.cli.sha384.{arch} missing for platform linux/{arch}"
                )
            build_args[f"CHISEL_SHA384_{arch.upper()}"] = digest

    build_args["CHISEL_RELEASE_REPOSITORY"] = chisel["release"]["repository"]
    build_args["CHISEL_RELEASE_REF"] = chisel["release"]["commit"]
    build_args["CHISEL_RELEASE_PATH"] = chisel["release"]["path"]
    build_args["CHISEL_SLICES"] = " ".join(chisel["slices"])
...
def render_docker_buildx(invocation: dict[str, Any]) -> str:
    ...
    spaces (notably CHISEL_SLICES) survive intact when read into a bash array.
    """
