...
4. `tests/runtime-hardening.sh` - exports the rootfs of the built image and
   asserts no shell, no apt/curl/wget, no apt state, non-root runtime user,
   and the expected entrypoint.
...
1. Pin the Ubuntu builder image by digest.
2. Choose the Chisel CLI version and record the tarball SHA384 for every
   architecture in `image.platforms`. Canonical publishes per-architecture
   tarballs with distinct digests, so the manifest stores them under
   `chisel.cli.sha384.<arch>`.
3. Pin a `canonical/chisel-releases` commit from the intended Ubuntu release
   branch.
4. Choose the minimum Chisel slices needed by the application.
5. Place the per-platform application artifacts under `dist/`, or update
   `.dockerignore` with the minimum build-context paths needed by the
   downstream builder.
6. Verify the application artifacts and record the per-platform paths and
...
The Dockerfile selects the matching Chisel checksum, application artifact path,
and application SHA256 based on `TARGETARCH`, and fails fast if the selected
architecture's value is empty or points outside the build context.
