...
`NWarila/chiseled-application-template`.
...
gh repo create OWNER/IMAGE_REPO \
  --template NWarila/chiseled-application-template \
  --public \
  --clone
```

The GitHub UI path is also fine: choose **Use this template** on
`NWarila/chiseled-application-template`, create the downstream repository, and
...
4. Keep `containers/Dockerfile` focused on the Chisel root filesystem and the
   final runtime assembly. If the application needs a different entrypoint,
   change `application.binary_path`, `runtime.entrypoint`, and the Dockerfile
   entrypoint together.
...
calls the local `.github/workflows/reusable-chisel-image-build.yaml`, so a new
...
jobs:
  image-build:
    ...
    permissions:
      contents: read
    uses: NWarila/chiseled-application-template/.github/workflows/reusable-chisel-image-build.yaml@0123456789abcdef0123456789abcdef01234567
    with:
