...
- Dockerfile contract markers that protect the Chisel build pattern.
...
4. Building and loading the OCI image for `linux/amd64`.
...
breakage in the Chisel pin chain, the Dockerfile, or the build helpers
surfaces immediately as a CI failure on the template itself.
